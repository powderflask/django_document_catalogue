...
def permission_required(permission_fn):
    """
        Constructs a CBV decorator that checks permission_fn(view.request.user, view.kwargs) before calling view.dispatch
          to test if request.user has a given (object) permission.
